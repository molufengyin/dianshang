<template>
    <div class="add">
        <x-header>  <span style="color:#fff;font-size:16px;">修改商品信息</span>  <a href="#/" style="color:#aaa;font-size:18px;" slot="right"> <i class="fa fa-home"></i> </a></x-header>
        <form ref="myform" action="">
           ...
           <button @click.stop="edit($event)" type="submit"> 上传 </button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            edit(event){
                if(this.name == ''){
                   return alert('商品名称不能为空哦！');
                }
                if(this.price == ''){
                   return alert('请给我一个价格！');
                }
                if(this.des == ''){
                   return alert('描述一下我吧！');
                }
                ...
                this.$axios.post(`/goods/edit/${id}`,formDate,config).then((good)=>{
                    console.log(111);
